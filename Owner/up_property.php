      ...
      <div class="row">
        ...
        <div class="col-lg-6 col-md-6 d-flex flex-column align-items-center justify-content-center pt-2">
          <div class="card ">
            ...
            <?php 
              $urlId = base64_decode($_GET['p']);
              $dataQQ = $isPerformOwnerOBJ->getPropertyToEdit($urlId);
              foreach ($dataQQ as $key => $value) {
                // code...
            ...
            ?>
                <div class="card-body">
                  ...
                  <div class="pt-4 pb-2">
                    ...
                    <h5 class="card-title text-center pb-0 fs-4">Update Real Real State (Site Registration)</h5>
                  ...
                  <form class="row g-0 needs-validations" novalidate method="post" action="" id="forms">
                    ...
                    <p class="text-center text-success spanError errorALL"><?= $isPerformOwnerOBJ->isUpdateRealstate(); ?></p>

                    <div class="col-md-12">
                      <label for="yourUsername" class="col-form-label-sm">Property Type *</label>
                      ...
                      <span class="text-danger spanError spanError-propertyType"></span>
                    </div>
                     ...
                     <div class="col-md-12">
                      ...
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="Address" class="form-control" id="address" value="<?=$PropertyAddress; ?>">
                     ...
                     <div class="col-md-12">
                      <label for="yourUsername" class="col-form-label-sm">Propery Size *</label>
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="propertySize" class="form-control" id="propertySize" value="<?=$PropertySize;  ?>">
                      </div>
                      <span class="text-danger spanError spanError-propertySize"></span>
                    </div>

                    <div class="col-md-12">
                      <label for="yourUsername" class="col-form-label-sm">Block *</label>
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="block" class="form-control" id="block" value="<?=$Blocks;  ?>">
                      </div>
                      <span class="text-danger spanError spanError-block"></span>
                    </div>

                     <div class="col-md-12">
                      <label for="yourUsername" class="col-form-label-sm">Manager Name *</label>
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <select id="managerEmail" name="managerEmail" class="form-select">
                          <option value="Choose">Choose...</option>

                          <?php 
                            $dataUserList = $isPerformOwnerOBJ->getManager($_SESSION['sessionID']);
                            $rowUserList = $isPerformOwnerOBJ->getManager($_SESSION['sessionID']);
                            if ($rowUserList > 0) {
                      ...
                      <span class="text-danger spanError spanError-managerEmail"></span>
                    </div>

                    <div class="col-12 mt-5">
                      <button class="btn btn-primary w-100" type="submit" name="UpdateRealstate" id="RegisterProperty">Update Property</button>
                    </div>
                  </form>
...
$(document).ready(function() {
    ...
    $("#propertyType").on('blur keyup change', function() {
        var propertyType = $(this).val();
        if (propertyType === 'Choose') {
            $(this).css("border-color","red");
            $(".spanError-propertyType").html("* This field is required.");
        } else {
            $(this).css("border-color","green");
            $(".spanError-propertyType").html('');
        }
    });

    // Inline validation on blur / keyup
    $("#address").on('blur keyup change', function() {
        var address = $(this).val().trim();
        if (address === '') {
            $(this).css("border-color","red");
            $(".spanError-address").html("* This field is required.");
        }else {
            $(this).css("border-color","green");
            $(".spanError-address").html('');
        }
    });

    // Inline validation on blur / keyup
    $("#propertySize").on('blur keyup change', function() {
        var propertySize = $(this).val().trim();
        if (propertySize === '') {
            $(this).css("border-color","red");
            $(".spanError-propertySize").html("* This field is required.");
        }else {
            $(this).css("border-color","green");
            $(".spanError-propertySize").html('');
        }
    });

     // Inline validation on blur / keyup
    $("#block").on('blur keyup change', function() {
        var block = $(this).val().trim();
        if (block === '') {
            $(this).css("border-color","red");
            $(".spanError-block").html("* This field is required.");
        }else {
            $(this).css("border-color","green");
            $(".spanError-block").html('');
        }
    });


      $("#managerEmail").on('blur keyup change', function() {
        var managerEmail = $(this).val();
        if (managerEmail === 'Choose') {
            $(this).css("border-color","red");
            $(".spanError-managerEmail").html("* This field is required.");
        } else {
            $(this).css("border-color","green");
            $(".spanError-managerEmail").html('');
        }
    });
    

    // Final validation on submit
    $("#RegisterProperty").on('click', function(event) {
        // Trigger all validations
        $("#address, #propertySize, #block, #propertyType, #managerEmail").trigger('change');
        // $("#propertyType, #managerEmail").trigger('change');

        // Prevent submission if any error remains
        if ($(".spanError-address").html() || $(".spanError-block").html() || $(".spanError-managerEmail").html() || $(".spanError-propertySize").html()  || $(".spanError-propertyType").html()) {
