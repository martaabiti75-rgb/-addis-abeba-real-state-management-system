                ...
                <div class="card-body">
                  ...
                  <div class="pt-4 pb-2">
                    ...
                    <?php 
                      ...
                      $dataQ = $isPerformOwnerOBJ->getRoomToEdit($urlId);
                    ...
                    ?>
                  <h5 class="card-title text-center pb-0 fs-4">Update Room (Site Registration)</h5>
                    
                  </div>
                  <!--class: needs-validation attribute:novalidate -->
                  <form class="row g-0 needs-validations" novalidate method="post" action="" enctype="multipart/form-data">
                    
                    
                    <p class="text-center text-success spanError errorALL"><?= $isPerformOwnerOBJ->isUpdateRoom(); ?></p>
                     
                     <div class="col-md-12">
                      <label for="yourUsername" class="col-form-label-sm">Room Number *</label>
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="RoomNumber" class="form-control" id="RoomNumber" placeholder="Enter Room number" value="<?=$RoomNumber; ?>">
                      </div>
                     ...
                     <div class="col-md-12">
                      ...
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="roomSize" class="form-control" id="roomSize" placeholder="Enter Room Size" value="<?=$RoomSize; ?>">
                      </div>
                    ...
                    <div class="col-md-12">
                      ...
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="floornumber" class="form-control" id="floornumber" placeholder="Enter Floor number" value="<?=$FloorNumber; ?>">
                      </div>
                     ...
                     <div class="col-md-12">
                      ...
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="blobknumber" class="form-control" id="blobknumber" placeholder="Enter Floor number" value="<?=$BlockNo; ?>">
                      </div>
                    ...
                    <div class="col-md-12">
                      ...
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="saleprice" class="form-control" id="saleprice" placeholder="Enter Floor number" value="<?=$SalePrice; ?>">
                      </div>
                     ...
                     <div class="col-md-12">
                      ...
                      <div class="input-group has-validation">
                        ...
                        <select id="realstateid" name="realstateid" class="form-select">
                          ...
                          ?>
                          <option value="<?=$value['RealStateId']; ?>"><?=$value['RealStateId']; ?></option>
                          <?php } } ?>
                        </select>
                      </div>
                    ...
                    <div class="col-12 mt-5">
                      <button class="btn btn-primary w-100" type="submit" name="isUpdateRoom" id="isUpdateRoom">Update Room</button>
                    </div>
                  </form>
...
$(document).ready(function() {
    ...
    // Inline validation on blur / keyup
    $("#RoomNumber").on('blur keyup change', function() {
        var RoomNumber = $(this).val().trim();
        if (RoomNumber === '') {
            $(this).css("border-color","red");
            $(".spanError-RoomNumber").html("* This field is required.");
        }else {
            $(this).css("border-color","green");
            $(".spanError-RoomNumber").html('');
        }
    });

    // Inline validation on blur / keyup
    $("#roomSize").on('blur keyup change', function() {
        var roomSize = $(this).val().trim();
        if (roomSize === '') {
            $(this).css("border-color","red");
            $(".spanError-roomSize").html("* This field is required.");
        }else {
            $(this).css("border-color","green");
            $(".spanError-roomSize").html('');
        }
    });

     // Inline validation on blur / keyup
    $("#floornumber").on('blur keyup change', function() {
        var floornumber = $(this).val().trim();
        if (floornumber === '') {
            $(this).css("border-color","red");
            $(".spanError-floornumber").html("* This field is required.");
        }else {
            $(this).css("border-color","green");
            $(".spanError-floornumber").html('');
        }
    });

      // Inline validation on blur / keyup
    $("#blobknumber").on('blur keyup change', function() {
        var blobknumber = $(this).val().trim();
        if (blobknumber === '') {
            $(this).css("border-color","red");
            $(".spanError-blobknumber").html("* This field is required.");
        }else {
            $(this).css("border-color","green");
            $(".spanError-blobknumber").html('');
        }
    });

        // Inline validation on blur / keyup
    $("#saleprice").on('blur keyup change', function() {
        var saleprice = $(this).val().trim();
        if (saleprice === '') {
            $(this).css("border-color","red");
            $(".spanError-saleprice").html("* This field is required.");
        }else {
            $(this).css("border-color","green");
            $(".spanError-saleprice").html('');
        }
    });


    $("#realstateid").on('blur keyup change', function() {
        var realstateid = $(this).val();
        if (realstateid === 'Choose') {
            $(this).css("border-color","red");
            $(".spanError-realstateid").html("* This field is required.");
        } else {
            $(this).css("border-color","green");
            $(".spanError-realstateid").html('');
        }
    });

      $("#files").on('blur keyup change', function() {
        var files = $(this).val();
        if (files === '') {
            $(this).css("border-color","red");
            $(".spanError-files").html("* This field is required.");
        } else {
            $(this).css("border-color","green");
            $(".spanError-files").html('');
        }
    });
    ...
    $("#isUpdateRoom").on('click', function(event) {
        // Trigger all validations
        $("#RoomNumber,  #files, #blobknumber, #saleprice, #roomSize, #floornumber, #realstateid").trigger('change');
        // $("#propertyType, #realstateid").trigger('change');

        // Prevent submission if any error remains
        if ($(".spanError-RoomNumber").html() || $(".spanError-saleprice").html() || $(".spanError-files").html() || $(".spanError-floornumber").html() || $(".spanError-blobknumber").html() || $(".spanError-realstateid").html() || $(".spanError-roomSize").html()) {
