   ...
   <section class="section dashboard">
      <div class="row">
       ...
       <?php
        ...
        // echo $urlId;
        $dataQ2 = $isPerformOwnerOBJ->getOWInfobyId($urlId);
        $rowQ2 = $isPerformOwnerOBJ->getOWInfobyId($urlId);

        foreach ($dataQ2 as $key => $value) {
          // code...
          ...
          <div class="card ">
                ...
                <div class="card-body">
                  ...
                  <div class="pt-4 pb-2">
                    ...
                    <h5 class="card-title text-center pb-0 fs-4">Request for Real Estate Owner </h5>
                    
                  </div>
                   <p class="text-center text-success"> <?= $CommonOBJ->isUpdateOWInfo(); ?></p>
                  <!--class: needs-validation attribute:novalidate -->
                  <!-- <form class="row g-3" novalidate method="post" action="" id="forms"> -->
                  
                  <form class="row g-3" method="post" action="" id="ownerForm">
                    <div class="col-md-12">
                      <label for="yourUsername" class="col-form-label-sm">Address 1 *</label>
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="Address" class="form-control" id="address" value="<?=$address_line1; ?>">
                        <input type="hidden" name="email" class="form-control" id="email" value="<?=$_SESSION['sessionID'] ?>">
                        <input type="hidden" name="urlid" class="form-control" id="urlid" value="<?=$urlId; ?>">
                        <input type="hidden" name="fullname" class="form-control" id="fullname" value="<?=$fullname; ?>">
                      </div>
                      <span class="text-danger spanError" id="spanError-address"></span>
                    </div>
                     ...
                     <div class="col-md-12">
                      ...
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" name="Address2" class="form-control" id="address2" value="<?=$address_line2; ?>" placeholder="Address 2" onscroll="City">
                      </div>
                      <span class="text-danger spanError " id="spanError-address2"></span>
                    </div>
                    ...
                    <div class="col-md-12">
                      ...
                      <div class="input-group has-validation">
                        <span class="input-group-text" id="inputGroupPrepend">@</span>
                        <input type="text" class="form-control" name="City" id="city" value="<?=$city; ?>" placeholder="City" onscroll="City">
                      </div>
                      <span class="text-danger spanError " id="spanError-city"></span>
                    </div>

                     <div class="form-group">
                      <label for="exampleInputEmail1">Phone Number</label>
                      <input type="text" class="form-control" id="phone" placeholder="phone" name="phone" value="<?=$phone_number; ?>">
                     ...
                     <div class="col-md-12">
                      ...
                      <span class="text-danger spanError " id="spanError-region"></span>
                    </div>
                     ...
                     <div class="col-md-12">
                      ...
                      <span class="text-danger spanError " id="spanError-gender"></span>
                    </div>
                    ...
                    <div class="col-12">
                      <button class="btn btn-primary w-100" type="submit" name="isUpdateOWInfo" id="isRequestForOwner">Submit</button>
                    </div>
                  </form>
...
$(function () { // shorthand for document.ready
  ...
  // validate helpers (kept your original logic, slightly cleaned)
  function validateAddress() {
      let v = $("#address").val().trim();
      if (v === "") {
          $("#spanError-address").text("* Address is required.");
          $("#address").css("border-color", "red");
          return false;
      } else {
          $("#spanError-address").text("");
  ...
  function validateAddress2() {
      let v = $("#address2").val().trim();
      if (v === "") {
          $("#spanError-address2").text("* Address 2 is required.");
          $("#address2").css("border-color", "red");
          return false;
      } else {
          $("#spanError-address2").text("");
  ...
  function validateCity() {
      let v = $("#city").val().trim();
      if (v === "") {
          $("#spanError-city").text("* City is required.");
  ...
  function validateRegion() {
      let v = $("#region").val();
      if (v === "Choose" || v === null) {
          $("#spanError-region").text("* Please select region.");
          $("#region").css("border-color", "red");
          return false;
      } else {
          $("#spanError-region").text("");
  ...
  // Inline triggers
  $("#address").on("blur keyup", validateAddress);
  $("#address2").on("blur keyup", validateAddress2);
  $("#city").on("blur keyup", validateCity);
  $("#region").on("change blur", validateRegion);
  $("#gender").on("change blur", validateGender);
  $(".fayda-input").on("input", validateFayda);

  // Fayda multiple-input helper (keeps your auto-advance/backspace behavior)
  ...
  inputs.forEach((input, index) => {
      input.addEventListener("input", () => {
          ...
          // update hidden
          let full = Array.from(inputs).map(i => i.value || "").join("");
  ...
  // IMPORTANT: bind submit to the FORM (not the button)
 $("#ownerForm").on("submit", function (evt) {

    let a = validateAddress();
    let b = validateAddress2();
    let c = validateCity();
    let d = validateRegion();
    let gOk = validateGender();
    let p = validatePhone();
    let f = validateFayda();
    ...
    if (!a || !b || !c || !d || !gOk || !p || !f) {
        ...
        if (!a) { $("#address").focus(); }
        else if (!b) { $("#address2").focus(); }
        else if (!c) { $("#city").focus(); }
        else if (!d) { $("#region").focus(); }
        else if (!gOk) { $("#gender").focus(); }
        else if (!p) { $("#phone").focus(); }
        else if (!f) { $(".fayda-input").first().focus(); }
